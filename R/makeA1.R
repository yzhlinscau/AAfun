makeA1=function (pedigree) 
{
  ...
  Tinv@p <- as.integer(c(match(1:N, Tinv.col[el.order]), length(el.order) + 
                           1) - 1)
  Tinv@x <- as.double(Tinv.x[el.order])
  nA <- N + length(dnmiss) + length(snmiss)
  nA <- nA + sum(duplicated(paste(nPed[, 2], nPed[, 3])[bnmiss]) == 
                   FALSE)
  inbreeding <- c(rep(0, N), -1)
  nPed[nPed == -998] <- N + 1
  Cout <- .C("acinv", as.integer(nPed[, 2] - 1), as.integer(nPed[, 
                                                                 3] - 1), as.double(inbreeding), as.integer(Tinv@i), as.integer(c(Tinv@p, 
                                                                                                                                  length(Tinv@x))), as.double(Tinv@x), as.integer(N), as.integer(length(Tinv@x)), 
             as.integer(rep(0, nA)), as.integer(rep(0, N + 1)), as.double(rep(0, 
                                                                              nA)), as.integer(nA))
  Ainv <- Matrix(0, N, N)
  Ainv[1, 2] <- 1
