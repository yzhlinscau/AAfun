...
panel.conf <- function(x, y, corr=NULL, col.regions, cor.method, digits=2, cex.cor, ...){
  ...
  if(!is.null(corr)) {
    ...
    ci <- paste(est, prefix, sep="\n")
    if(auto) cex.cor <- 0.6/strwidth(ci)
    text(0.5, 0.3, ci, cex=cex.cor, col=color)
  }
}
